...
public class DiscoveryDAOImpl implements DiscoveryDAO {
    ...
    private static final String READ_ALL_DISCOVERY =
            "SELECT user.user_id, username, email, is_active, password, discovery_id, name, description, url, date, "
            + "up_vote, down_vote FROM discovery LEFT JOIN user ON discovery.user_id = user.user_id;";

    private static final String READ_DISCOVERY =
            "SELECT user.user_id, username, email, password, discovery_id, name, description, url, date, up_vote, " +
                    "down_vote "
    ...
    private static final String UPDATE_DISCOVERY =
            "UPDATE discovery SET name = :name, description = :description, url = :url, user_id = :user_id, " +
                    "date = :date, up_vote = :up_vote, down_vote = :down_vote "
    ...
    public List<Discovery> getAll() {
        List<Discovery> discoveries = template.query(READ_ALL_DISCOVERY, new DiscoveryRowMapper());
        return discoveries;
    }
